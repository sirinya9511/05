...
app.get("/api/health", (req: Request, res: Response) => {
  res.json({ status: "ok", service: "Safety Hazard RA01 API (Vercel Serverless Ready)" });
});

// AI Hazard Identification & Unforeseen Risk Analysis Endpoint
app.post("/api/gemini/analyze-hazard", async (req: Request, res: Response) => {
  try {
    const {
      activity,
      location,
...
app.post("/api/gemini/generate-checklist", async (req: Request, res: Response) => {
  try {
